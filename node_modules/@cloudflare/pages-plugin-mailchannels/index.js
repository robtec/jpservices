...
var textHTMLContent = ({ request, formData, name }) => {
  ...
  <html>
    <body>
      <div>Hi John, you received a request to call back a customer:</div>
      <hr>
      <table>
      <tbody>
      ...
      <hr>
      <div>This request was submitted via your website.</div>
    </body>
  </html>`;
};
var onRequest = async (context) => {
  ...
  return await template_plugin_default({
    respondWith: async ({ formData, name }) => {
      ...
      const subject = typeof pluginArgs.subject === "function" ? pluginArgs.subject(submission) : pluginArgs.subject || `Call Back Request ${new Date().toLocaleString('en-IE', { timeZone: 'Europe/Dublin' })}`;
